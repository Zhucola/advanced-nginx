...
```
如果为on，当静态资源发生404的时候，会在error_log日志中记录error级别的错误信息
```
 2019/05/28 16:42:17 [error] 2941#0: *9 open() "/tmp/aa.php" failed (2: No such file or directory), client: 127.0.0.1, server: , request: "GET /aa.php HTTP/1.1", host: "127.0.0.1"

```
即使为off，在请求fastcgi情况下发生404，也会记录错误信息
```
2019/05/28 16:41:24 [error] 2923#0: *5 FastCGI sent in stderr: "Primary script unknown" while reading response header from upstream, client: 127.0.0.1, server: , request: "GET /aa.php HTTP/1.1", upstream: "fastcgi://127.0.0.1:9000", host: "127.0.0.1"
```
